...
from .views import OrgListView, UserAskView, OrgDetailHomeView, OrgDetailCourseView, OrgDetailDescView, \
    OrgDetailTeacherView, AddUserFavView,TeacherListView,TeacherDetailView
...
urlpatterns = [
    ...
    # 某个课程机构的教师页 需要机构的id
    url(r'^org_teacher/(?P<org_id>[0-9]+)/$', OrgDetailTeacherView.as_view(), name='org_detail_teacher'),
    ...
    url(r'^teacher/detail/(?P<teacher_id>[0-9]+)/$',TeacherDetailView.as_view(), name='teacher_detail')
]
